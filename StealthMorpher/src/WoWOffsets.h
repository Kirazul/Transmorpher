...
static const uint32_t UNIT_FIELD_BYTES_0 = 0x17 * 4; // 0x5C: b0 race, b1 class, b2 gender, b3 power
...
// Player appearance customization (a player's skin/face/hair come from these,
// NOT from the display id — so cloning a player needs these copied too).
// Field index = UNIT_END(0x94) + offset; byte offset = index*4. Verified against
// Enums.h (PLAYER_BYTES = UNIT_END+0x05; PLAYER_CHOSEN_TITLE = UNIT_END+0xAD = 0x141 ✓).
static const uint32_t PLAYER_FIELD_BYTES  = 0x99 * 4; // 0x264: b0 skin, b1 face, b2 hairStyle, b3 hairColor
static const uint32_t PLAYER_FIELD_BYTES2 = 0x9A * 4; // 0x268: b0 facialHair (b1 restState/b2.. unused here)
