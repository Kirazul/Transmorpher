...
// True if `unit` is currently riding a vehicle (passenger or controller). Mirrors the
// client's own Lua_UnitInVehicle (0x006133D0, RE-verified against this wow.exe): the unit
// carries a vehicle-component pointer at unit+0xF60 and the component's +0x14 state == 3
// while seated. No hardcoded creature/spell ids; works for every vehicle (Oculus/Nexus
// drakes, EoE drakes, ToGC mounts, gunship cannons, encounter vehicles, etc.). While this
// is true we must NOT force a morph display/scale/mount or rebuild the model, or the engine's
// vehicle/seat presentation breaks and the rider goes invisible.
...
// Ensures the user's WTF\Config.wtf contains SET dbCompress "0" (adds or rewrites it).
// Required for reliable client-side DB/visual handling. Backs up the original once.
