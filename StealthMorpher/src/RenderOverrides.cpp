...
// True only while WE are actively forcing a far-clip override into the engine global.
// When the override is OFF and clean we never write the global at all, so the engine's
// own world-streaming distance is left untouched (no per-tick re-stream, no pink/double
// reload on world entry — this is the default state).
static bool g_worldFarClipDirty = false;
static float g_worldFarClipWritten = 0.0f;
...
static void SyncWorldState() {
    ...
    // This runs EVERY tick and on EVERY world entry. The engine writes the world
    // far-clip global itself while streaming a zone, so writing it here when we have
    // no override active was overriding the engine's default mid-stream — that is the
    // "world re-streams / pink flash / double reload on enter" bug, and it happened
    // even with the addon disabled (the proxy DLL's tick always runs). So: only touch
    // the global when the user explicitly enabled the far-clip override; otherwise
    // leave the engine entirely alone. When the override is turned off, restore the
    // captured original exactly ONCE, then stop writing.
    __try {
        if (cfg.worldFarClipEnabled) {
            ...
            const float farClip = ClampFarClip(cfg.worldFarClip);
            if (!g_worldFarClipDirty || g_worldFarClipWritten != farClip) {
                WriteWorldFarClip(farClip);
                g_worldFarClipWritten = farClip;
                g_worldFarClipDirty = true;
            }
        } else if (g_worldFarClipDirty) {
            if (g_worldOriginalFarClipCaptured) {
                WriteWorldFarClip(GetSafeFarClip(g_worldOriginalFarClip));
            }
            g_worldFarClipDirty = false;
        }
        // Disabled & clean: never write the far-clip global -> engine default intact.
