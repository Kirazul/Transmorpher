...
namespace {
    ...
    // Per-call scratch for morph rows. When morphing across different cast mechanics
    // (e.g. Fireball -> a channel/instant spell), the target's visual content may live
    // in a phase the source never fires (channelKit/impactKit) so nothing shows. We
    // synthesize a copy that also puts the target's primary kit into the source's cast
    // phase, so ANY spell -> ANY spell renders something. Never mutates the live row.
    static SpellVisualRec g_morphScratch[8] = {};
    static unsigned g_morphScratchIdx = 0;
    ...
    // Resolves the target spell's primary SpellVisual id. UNLIMITED compatibility:
    // any spell can morph into any other. We take the target's own primary visual
    // (slot 0, fallback slot 1) verbatim — no "compatibility" scoring that used to
    // reject mismatched profiles (e.g. Fireball -> Mind Flay / a missile). Spells not
    // in the preloaded addon DBC are resolved straight from the LIVE client Spell.dbc
    // so EVERY spell in the game is a valid target.
    static uint32_t SelectCompatibleTargetVisualId_NoLock(uint32_t /*sourceSpellId*/, uint32_t /*sourceVisualId*/, uint32_t targetSpellId) {
        ...
        uint32_t targetV0 = 0, targetV1 = 0;
        auto targetIt = g_spellIdToVisualIdMap.find(targetSpellId);
        if (targetIt != g_spellIdToVisualIdMap.end()) {
            targetV0 = targetIt->second.first;
            targetV1 = targetIt->second.second;
        }

        // Live client DBC fallback: read the target spell's SpellVisualID[0]/[1]
        // (fields 131/132) directly so any spell works, even outside the addon DBC.
        if (targetV0 == 0 && targetV1 == 0) {
            void* db = *reinterpret_cast<void**>(ADDR_SPELL_DB);
            if (db) {
                void* rec = GetRow(db, targetSpellId);
                if (rec) {
                    targetV0 = *reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(rec) + 131 * 4);
                    targetV1 = *reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(rec) + 132 * 4);
                }
            }
        }

        if (targetV0 > 0 && targetV0 < 100000) return targetV0;
        if (targetV1 > 0 && targetV1 < 100000) return targetV1;
        return 0;
    }
    ...
    // Surgically clears ONLY the spell-visual kits selected by the active
    // per-category optimization flags (precast/cast/channel/missile/etc.) on a
    // live DBC row, leaving everything else intact. The row is first reset to
    // the clean retail snapshot so repeated toggles never accumulate, and the
    // change is tracked so SoftResetCache()/RestoreAllSanitizedRows_NoLock()
    // can fully restore it when toggles change. Caller must hold the exclusive
    // lock. Hide-all is now handled by SpellClass as per-category clearing, not
    // by nulling whole visuals.
    static bool SanitizeLiveRowGranular_NoLock(SpellVisualRec* row) {
        if (!row || row == &g_nullVisualRec) return false;
        if (reinterpret_cast<uintptr_t>(row) < 0x10000) return false;
        if (g_backupDataPtrMap.count(row)) return false; // never touch addon-DBC backing rows

        // Already sanitized for the current toggle generation -> nothing to do.
        // SoftResetCache() bumps g_morphGeneration and restores rows whenever a
        // toggle changes, so this keeps the hot hook path cheap.
        auto genIt = g_sanitizedPtrGeneration.find(row);
        if (genIt != g_sanitizedPtrGeneration.end() && genIt->second == g_morphGeneration) {
            return true;
        }
        ...
        // Reset to the pristine retail bytes, then clear only the enabled kits.
        std::memcpy(row, it->second.data(), 128);
        const SpellVisualRec* original = reinterpret_cast<const SpellVisualRec*>(it->second.data());
        SanitizeSpellVisualRec(row, original);

        DWORD dummy;
        VirtualProtect(row, 128, oldProt, &dummy);

        g_sanitizedPtrGeneration[row] = g_morphGeneration;
        return true;
    }
    ...
    static void SynchronizeSpellVisualRow(SpellVisualRec* finalRow, bool granular, bool isProtected) {
        ...
        // Hide-all is handled by SpellClass per instance, so the shared underlying
        // row is left clean here.
        if (g_hideAllSpells) {
            RestoreSpellVisualRow(finalRow);
            return;
        }

        // Granular per-category filtering: surgically clear only the kits that
        // the active category toggles select (e.g. just the channel beam, just
        // the missile, just the impact), leaving the rest of the spell visible.
        SanitizeLiveRowGranular_NoLock(finalRow);
    }
    ...
    static SpellVisualRec* __cdecl Hooked_GetSpellVisualRow(SpellRec* pSpellRec) {
        ...
        // Spell morphs apply ONLY to the local player's own casts (host == you), so a
        // morph never changes the spell on other players casting the same spell.
        if (!isProtected && SpellClass_HostIsLocalPlayer()) {
            AcquireSRWLockShared(&g_spellMorphLock);
        ...
        if (targetVisualId > 0 && targetVisualId != sourceVisualId) {
            ...
            if (overrideRec && overrideRec != &g_nullVisualRec &&
                reinterpret_cast<uintptr_t>(overrideRec) > 0x10000) {
                // Synthesize the morph row so ANY target renders during the source's
                // cast. Compatible morphs (target already has a castKit) are copied
                // unchanged; cross-mechanic targets get their primary visual content
                // (channel/impact/precast/state kit) promoted into the cast phase.
                SpellVisualRec* s = &g_morphScratch[g_morphScratchIdx++ & 7];
                *s = *overrideRec;
                if (s->m_castKit == 0) {
                    int32_t primary = s->m_channelKit ? s->m_channelKit
                                    : s->m_impactKit ? s->m_impactKit
                                    : s->m_precastKit ? s->m_precastKit
                                    : s->m_casterImpactKit ? s->m_casterImpactKit
                                    : s->m_stateKit;
                    s->m_castKit = primary;
                }
                finalRow = s;
        ...
        // Per-caster-class filtering (SpellClass): hide the whole visual, or only
        // specific kits, based on the class of the unit the visual is played on.
        // Morphed and protected visuals are never hidden. Falls through to the
        // original row when nothing applies (fail-safe show).
        (void)granular;
        bool protectedHost = isManualMorph || protectFinal;
        return reinterpret_cast<SpellVisualRec*>(
            SpellClass_DecideRow(sourceSpellId, static_cast<uint32_t>(finalRow->m_ID),
                                 finalRow, &g_nullVisualRec, protectedHost));
    }
}
...
static SpellVisualRec* __fastcall Hooked_GetCastVisual(void* pThis, void* edx, SpellRec* pSpellRec) {
    ...
    if (pSpellRec && res && res != &g_nullVisualRec && reinterpret_cast<uintptr_t>(res) > 0x10000) {
        ...
        // Hide-all is handled by SpellClass_DecideRow as per-category clearing with
        // encounter/aura safety gates. Do not return a null visual here, or player-
        // carried boss debuffs can lose their entire flame/shadow visual.
    }
...
bool InstallSpellVisualHook() {
    ...
    *reinterpret_cast<DWORD*>(target + 1) =
    ...
    SpellClass_InstallEmitterHook(); // Per-caster-class host tracking (PlaySpellVisual_core)
...
// Reverse map visualId -> a representative spellId (built once from the preloaded
// spell->visual table). Prefers the lowest spell id for stable, recognisable names.
uint32_t SpellMorph_FindSpellForVisual(uint32_t visualId) {
    static std::unordered_map<uint32_t, uint32_t> s_visualToSpell;
    static bool s_built = false;
    if (!s_built) {
        for (auto& kv : g_spellIdToVisualIdMap) {
            uint32_t spellId = kv.first;
            uint32_t v0 = kv.second.first, v1 = kv.second.second;
            for (uint32_t v : { v0, v1 }) {
                if (v == 0) continue;
                auto it = s_visualToSpell.find(v);
                if (it == s_visualToSpell.end() || spellId < it->second)
                    s_visualToSpell[v] = spellId;
            }
        }
        s_built = true;
    }
    auto it = s_visualToSpell.find(visualId);
    return it == s_visualToSpell.end() ? 0u : it->second;
}

// --- Visibility Logic (Post-Namespace for SoftResetCache Access) ---

// The legacy left-column toggles are the SIMPLE "apply to all units" optimization:
// they set the SpellClass GLOBAL "what to hide" categories AND enable g_applyAll, so
// the chosen categories are suppressed on every non-protected unit (other players,
// pets, trash/elite/rare) — while self/group/totems/encounter-bosses/world effects
// always stay visible. The g_hide* mirrors back the Get* accessors and let us know
// when ALL toggles are off again (so we drop g_applyAll and stop hiding).
static void SyncLegacyApplyAll() {
    bool any = (g_hideAllSpells || g_hidePrecast || g_hideCast || g_hideChannel ||
                g_hideAuraStart || g_hideAuraEnd || g_hideImpact || g_hideImpactCaster ||
                g_hideTargetImpact || g_hideAreaInstant || g_hideAreaImpact ||
                g_hideAreaPersistent || g_hideMissile || g_hideMissileMarker ||
                g_hideSoundMissile || g_hideSoundEvent);
    SpellClass_SetApplyAll(any);
    if (any) SpellClass_SetMasterEnabled(true);
}
void SetHideAllSpells(bool hide)     { g_hideAllSpells = hide; SpellClass_SetSimpleHideAll(hide); SyncLegacyApplyAll(); }
void SetShowOwnSpells(bool show)     { g_showOwnSpells = show; }
void SetHidePrecast(bool hide)       { g_hidePrecast = hide;        SpellClass_SetSimpleCategory(SCAT_PRECAST, hide); SyncLegacyApplyAll(); }
void SetHideCast(bool hide)          { g_hideCast = hide;          SpellClass_SetSimpleCategory(SCAT_CAST, hide); SyncLegacyApplyAll(); }
void SetHideChannel(bool hide)       { g_hideChannel = hide;       SpellClass_SetSimpleCategory(SCAT_CHANNEL, hide); SyncLegacyApplyAll(); }
void SetHideAuraStart(bool hide)     { g_hideAuraStart = hide;     SpellClass_SetSimpleCategory(SCAT_AURA_START, hide); SyncLegacyApplyAll(); }
void SetHideAuraEnd(bool hide)       { g_hideAuraEnd = hide;       SpellClass_SetSimpleCategory(SCAT_AURA_END, hide); SyncLegacyApplyAll(); }
void SetHideImpact(bool hide)        { g_hideImpact = hide;        SpellClass_SetSimpleCategory(SCAT_IMPACT, hide); SyncLegacyApplyAll(); }
void SetHideImpactCaster(bool hide)  { g_hideImpactCaster = hide;  SpellClass_SetSimpleCategory(SCAT_IMPACT_CASTER, hide); SyncLegacyApplyAll(); }
void SetHideTargetImpact(bool hide)  { g_hideTargetImpact = hide;  SpellClass_SetSimpleCategory(SCAT_IMPACT_TARGET, hide); SyncLegacyApplyAll(); }
void SetHideAreaInstant(bool hide)   { g_hideAreaInstant = hide;   SpellClass_SetSimpleCategory(SCAT_AREA_INSTANT, hide); SyncLegacyApplyAll(); }
void SetHideAreaImpact(bool hide)    { g_hideAreaImpact = hide;    SpellClass_SetSimpleCategory(SCAT_AREA_IMPACT, hide); SyncLegacyApplyAll(); }
void SetHideAreaPersistent(bool hide){ g_hideAreaPersistent = hide;SpellClass_SetSimpleCategory(SCAT_AREA_PERSISTENT, hide); SyncLegacyApplyAll(); }
void SetHideMissile(bool hide)       { g_hideMissile = hide;       SpellClass_SetSimpleCategory(SCAT_MISSILE, hide); SyncLegacyApplyAll(); }
void SetHideMissileMarker(bool hide) { g_hideMissileMarker = hide; SpellClass_SetSimpleCategory(SCAT_MISSILE_MARKER, hide); SyncLegacyApplyAll(); }
void SetHideSoundMissile(bool hide)  { g_hideSoundMissile = hide;  SpellClass_SetSimpleCategory(SCAT_SOUND_MISSILE, hide); SyncLegacyApplyAll(); }
void SetHideSoundEvent(bool hide)    { g_hideSoundEvent = hide;    SpellClass_SetSimpleCategory(SCAT_SOUND_EVENT, hide); SyncLegacyApplyAll(); }
