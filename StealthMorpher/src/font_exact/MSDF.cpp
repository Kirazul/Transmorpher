...
namespace {
    ...
    // The MSDF atlas sampler slots (12..15) use a fixed sampler configuration
    // (CLAMP/CLAMP/LINEAR/LINEAR/NONE) for every glyph atlas. Re-issuing those 5
    // SetSamplerState calls for every atlas page of every text string on every
    // frame was a needless per-frame cost (a major source of text-heavy lag).
    // Instead we configure each slot once and only re-configure after a device
    // reset/recreate (which resets sampler state), tracked by this flag array.
    bool s_slotSamplerConfigured[16] = { false };

    void EnsureAtlasSamplerState(IDirect3DDevice9* device, uint32_t slot) {
        if (!device || slot >= 16 || s_slotSamplerConfigured[slot]) return;
        ...
        s_slotSamplerConfigured[slot] = true;
    }

    void InvalidateAtlasSamplerState() {
        for (int i = 0; i < 16; ++i) s_slotSamplerConfigured[i] = false;
    }
    ...
    void __fastcall CGxString__WriteGeometryHk(CGxString* pThis, void* edx, int destPtr, int index, int vertIndex, int vertCount) {
        ...
        for (uint32_t pageIdx = 0; pageIdx < fontHandle->GetAtlasPageCount(); ++pageIdx) {
            ...
            if (atlasTexture && atlasTexture->texture) {
                ...
                // Sampler state is constant for atlas slots; set once per slot and
                // re-apply only after a device reset (see InvalidateAtlasSamplerState).
                EnsureAtlasSamplerState(device, slot);
            }
        }
    ...
    int __cdecl FreeType_Done_FreeTypeHk(FT_Library library) {
        // If the process is already terminating, do not touch D3D textures,
        // msdfgen or FreeType state — those objects may be half-destroyed and
        // the OS will reclaim everything. Touching them here is what caused the
        // ERROR #132 ACCESS_VIOLATION on client close.
    ...
    int __cdecl FreeType_InitHk(void* memory, FT_Library* alibrary) {
        if (!MSDF::INITIALIZED) {
            ...
            D3D::RegisterOnDestroy([]() {
                ...
                InvalidateAtlasSamplerState();
            });

            // A device reset clears sampler state, so re-arm the one-time atlas
            // sampler configuration to be re-applied on the next text render.
            D3D::RegisterResetCallback([](IDirect3DDevice9*, D3DPRESENT_PARAMETERS*) {
                InvalidateAtlasSamplerState();
            });
