...
bool InstallUpdateDisplayInfoHook()
{
    ...
    // Target the VERIFIED CGUnit_C::UpdateDisplayInfo (0x0073E410, prologue
    // 55 8B EC 83 EC 10 — confirmed by disasm). We deliberately do NOT use the old
    // FindUpdateDisplayInfoHook pattern (it scanned for a different 81 EC/0x88-stack
    // function and could land on the wrong address). Prefer the resolved pointer;
    // fall back to the verified literal only if resolution failed.
    g_updateDisplayHookAddr = CGUnit_UpdateDisplayInfo
        ? (DWORD)(uintptr_t)CGUnit_UpdateDisplayInfo
        : (DWORD)(base + 0x33E410);
