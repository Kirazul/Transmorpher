...
local mainFrameTitle = "|cff00d4ffTransmorpher |cffffffff1.0.1|r"
...
do
    ...
    slotSubclasses[offHandSlot] = { 
        "1H Axe", "1H Mace", "1H Sword", "1H Dagger", "1H Fist",
        "OH Axe", "OH Mace", "OH Sword", "OH Dagger", "OH Fist", "Shield", "Held in Off-hand",
        "MH Axe", "MH Mace", "MH Sword", "MH Dagger", "MH Fist" }
...
local function IsModelChangingForm()
    ...
    if classFileName == "DRUID" then
        return true -- All druid forms (Bear, Cat, Travel, Moonkin, Tree, Aquatic) are model-changing
...
-- Sends a command directly to the DLL without updating 
-- the addon's internal state (TransmorpherCharacterState).
-- Useful for temporary resets (suspension).
...
local function ReapplyCurrentMorph()
    if not TransmorpherCharacterState then return end
    
    -- Passive Suspension: If in a model-changing form (Moonkin, Cat etc), we stop overriding
    -- to allow the native game form to show correctly.
...
do
    ...
    local lastMorphCheck = 0
    local lastMorphForm = -1
    local lastMorphMounted = false

    -- Reusable timer frame to avoid constant CreateFrame calls
    local reapplyTimer = CreateFrame("Frame")
    reapplyTimer:Hide()
    reapplyTimer:SetScript("OnUpdate", function(self, elapsed)
        self.timer = self.timer - elapsed
        if self.timer <= 0 then
            ReapplyCurrentMorph()
            self:Hide()
    ...
    local function TriggerReapply(delay)
        reapplyTimer.timer = delay or 0.05
        reapplyTimer:Show()
    end

    mainFrame:SetScript("OnEvent", function(self, event, ...)
        if event == "PLAYER_LOGIN" or event == "PLAYER_ENTERING_WORLD" or event == "ZONE_CHANGED_NEW_AREA" then
            -- Reset state tracking on login/zone so it definitely re-applies if needed
            lastMorphForm = GetShapeshiftForm()
            lastMorphMounted = IsMounted() or false
            TriggerReapply(event == "PLAYER_LOGIN" and 0.2 or 0.01)
            
        elseif event == "UNIT_MODEL_CHANGED" or event == "UPDATE_SHAPESHIFT_FORM" then
            local unit = ...
            if event == "UPDATE_SHAPESHIFT_FORM" or unit == "player" then
                local currentForm = GetShapeshiftForm()
                local currentMounted = IsMounted() or false
                
                -- Only re-apply if the absolute form or mount state changed.
                -- This PREVENTS refreshing the character on every minor buff/aura/particle gain.
                if currentForm ~= lastMorphForm or currentMounted ~= lastMorphMounted then
                    local wasSuspending = not IsModelChangingForm() -- State BEFORE the change (roughly)
                    lastMorphForm = currentForm
                    lastMorphMounted = currentMounted
        ...
        elseif event == "CHAT_MSG_ADDON" then
            ...
            if (prefix == addonMessagePrefix or prefix == "DressMe") then
                -- logic for older received appearances if still needed (optional)
            end
        end
    end)
end
...
-- Print load message
DEFAULT_CHAT_FRAME:AddMessage("|cff00d4ff<Transmorpher>|r v1.0.1 loaded. Type |cff00ff00/morph|r to open or use the button on your character frame.")
